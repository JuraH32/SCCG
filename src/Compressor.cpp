    ...
    static bool parseMatchLine(const std::string &ln, ParsedMatchInfo &out) {
        ...
        int r, l;
        if (!(iss >> kw >> r >> l >> nom)) {
            out.is_valid_pure_match = false;
            return false;
        }
        if (kw != "MATCH" || nom != "NOMISMATCH") {
            out.is_valid_pure_match = false;
            return false;
        }
    ...
    void Compressor::postprocess(std::string& temp_input_filepath, const std::string& final_output_filepath) {
        ...
        while (std::getline(temp_file_stream, line)) {
            ...
            if (parseMatchLine(line, parsed_line_info)) { // It's a "MATCH R L NOMISMATCH" line
        ...
        if (!final_output_stream.is_open()) {
            throw std::runtime_error("Postprocess: Failed to open final output file: "
                                    + final_output_filepath);
        }

        long long prev_end = -1;
        for (const auto &ln : lines_after_merge_pass) {
            ParsedMatchInfo pi;
            if (parseMatchLine(ln, pi)) {
                int delta = (prev_end < 0
                            ? pi.ref_pos
                            : pi.ref_pos - (prev_end + 1));
                final_output_stream << delta << "," << pi.length << "\n";
                prev_end = pi.ref_pos + pi.length - 1;
            } else {
                // raw mismatch run -> pass straight through
                final_output_stream << ln << "\n";
            }
        }

        final_output_stream.close();
        std::cout << "Postprocessing complete. Output at: "
                << final_output_filepath << std::endl;
    }
    ...
    // Detect if this is the lowercase‐positions call
    bool is_lowercase = (label_prefix.rfind("LOWERCASE", 0) == 0);

    // prev_excl_end is the first base *after* the previous segment
    int prev_excl_end = 0;
    ...
    for (const auto &pos : positions) {
        ...
        // If these are lowercase positions, adjust by “+1 to delta, –1 to length”
        // but on the very first segment only adjust the length.
        if (is_lowercase) {
            if (first_segment) {
                length -= 1;
            } else {
    ...
    void Compressor::saveAlignmentSegmentsToFile(
     ...
     for (const auto &segment : segments) {
         if (segment.is_match) {
             ...
             // buffer up this tiny mismatch
             mismatch_buf += segment.mismatched_sequence;
         }
     }
     // flush trailing mismatches
     if (!mismatch_buf.empty()) {
         oss << mismatch_buf << "\n";
     }
